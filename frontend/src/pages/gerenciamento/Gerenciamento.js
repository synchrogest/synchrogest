...
import { Container, Row, Col, Card, Button, Form, Table, Modal, ProgressBar, Spinner, Alert } from 'react-bootstrap';
import { FaPlus, FaTrash, FaCheck, FaTimes, FaUserPlus, FaCalendarAlt, FaListAlt, FaToggleOn, FaToggleOff, FaInfoCircle } from 'react-icons/fa';
...
const Gerenciamento = () => {
  const { user } = useAuth();
  const isAuthenticated = !!user; // Verificar se o usuário está autenticado
  ...
  const [novoGerenciamento, setNovoGerenciamento] = useState({
    ...
    data_inicio: '',
    data_conclusao: '',
    publico: true // Por padrão, novos gerenciamentos são públicos
  });
  ...
  useEffect(() => {
    const fetchGerenciamentos = async () => {
      try {
        ...
        // Se for erro de autenticação, não mostrar como erro, apenas definir modo somente leitura
  ...
  // Carregar usuários para permissões (apenas se autenticado)

  // useEffect(() => {
  //   const fetchUsuarios = async () => {
  //     if (!isAuthenticated) return; // Não buscar usuários se não estiver autenticado
  ...
  useEffect(() => {
  const fetchUsuarios = async () => {
    if (!isAuthenticated || !user.is_admin) return; // só admins
    try {
      const response = await api.get('/usuarios');
      setUsuarios(response.data);
    } catch (err) {
  ...
  const carregarGerenciamento = async (id) => {
    try {
      ...
      // Se for erro de autenticação, redirecionar para a lista
      if (err.response && (err.response.status === 401 || err.response.status === 403)) {
        setGerenciamentoAtual(null);
        setError('Este gerenciamento requer autenticação para ser visualizado.');
      } else {
  ...
  const handleCriarGerenciamento = async () => {
    if (!isAuthenticated) return; // Não permitir criar se não estiver autenticado
  ...
  const handleAdicionarItem = async () => {
    if (!isAuthenticated) return; // Não permitir adicionar se não estiver autenticado
    
    try {
      await api.post(`/gerenciamento/${gerenciamentoAtual.id}/itens`, novoItem);
  ...
  const handleAdicionarAcao = async () => {
    ...
    try {
      await api.post(`/gerenciamento/${gerenciamentoAtual.id}/acoes`, novaAcao);
      await carregarGerenciamento(gerenciamentoAtual.id);
  ...
  const handleAdicionarPermissao = async () => {
    ...
    try {
      await api.post(`/gerenciamento/${gerenciamentoAtual.id}/permissoes`, novaPermissao);
      await carregarGerenciamento(gerenciamentoAtual.id);
  ...
  const handleRemoverPermissao = async (usuarioId) => {
    if (!isAuthenticated) return; // Não permitir remover se não estiver autenticado
  ...
  const handleToggleChecklist = async (checklistId, concluido) => {
    if (!isAuthenticated) return; // Não permitir atualizar se não estiver autenticado
  ...
  const handleToggleAtivoChecklist = async () => {
    if (!isAuthenticated || !checklistParaToggle) return; // Não permitir atualizar se não estiver autenticado
  ...
  const handleExcluirItem = async () => {
    if (!isAuthenticated) return; // Não permitir excluir se não estiver autenticado
  ...
  const temPermissaoEditar = () => {
    if (!gerenciamentoAtual || !isAuthenticated) return false;
      ...
      return (
        <Alert variant="info" className="my-3">
          {isAuthenticated 
            ...
            : "Nenhum gerenciamento público encontrado. Faça login para criar ou visualizar gerenciamentos privados."}
        </Alert>
      );
    }
          ...
          <Col md={4} key={gerenciamento.id} className="mb-4">
            <Card className="h-100 shadow-sm">
              <Card.Body>
                ...
                {gerenciamento.publico && (
                  <div className="small text-success mb-2">
                    <FaInfoCircle className="me-1" /> Público
                  </div>
                )}
              </Card.Body>
  ...
  const renderGerenciamentoDetalhes = () => {
    ...
    return (
      <div className="mt-4">
        {/* Alerta para usuários não autenticados */}
        {!isAuthenticated && (
          <Alert variant="info" className="mb-3">
            <FaInfoCircle className="me-2" /> 
            Você está visualizando no modo somente leitura. Faça login para editar ou criar novos gerenciamentos.
          </Alert>
        )}
        ...
        <Card className="mb-4 shadow-sm">
          ...
          <Card.Body>
            <Row className="mb-4">
              <Col md={8}>
                ...
                <div className="mt-3">
                  ...
                  {gerenciamentoAtual.publico && (
                    <div className="mt-2 text-success">
                      <FaInfoCircle className="me-1" /> Este gerenciamento é público
                    </div>
                  )}
                </div>
              </Col>
            ...
            {/* Botões de ação - visíveis apenas para usuários autenticados com permissão */}
            {isAuthenticated && podeEditar && (
              <Row className="mb-3">
                        ...
                        {gerenciamentoAtual.acoes.map(acao => {
                          ...
                          return (
                            <td key={`${item.id}-${acao.id}`} className="text-center">
                              {checklist ? (
                                checklist.ativo ? (
                                  <Button
                                    ...
                                    onClick={() => isAuthenticated && podeEditar && handleToggleChecklist(checklist.id, checklist.concluido)}
                                    disabled={!isAuthenticated || !podeEditar}
                                  >
                              ...
                              {isAuthenticated && podeEditar && checklist && (
                                <Button
                                  variant="link"
                                  size="sm"
  ...
  // Modal para criar novo gerenciamento - visível apenas para usuários autenticados
      ...
      <Modal.Body>
        <Form>
          ...
          <Form.Group className="mb-3">
            <Form.Check 
              type="checkbox"
              id="gerenciamento-publico"
              label="Tornar público (visível sem autenticação)"
              checked={novoGerenciamento.publico}
              onChange={(e) => setNovoGerenciamento({...novoGerenciamento, publico: e.target.checked})}
            />
          </Form.Group>
        </Form>
      </Modal.Body>
  ...
  return (
    <Container fluid className="py-4">
      {/* Alerta para usuários não autenticados na página principal */}
      {!gerenciamentoAtual && !isAuthenticated && (
        <Alert variant="info" className="mb-3">
          <FaInfoCircle className="me-2" /> 
          Você está visualizando no modo somente leitura. Faça login para editar ou criar novos gerenciamentos.
        </Alert>
      ...
      {!gerenciamentoAtual ? (
        <>
          <div className="d-flex justify-content-between align-items-center">
            ...
            {/* Botão de novo gerenciamento - visível apenas para usuários autenticados */}
            {isAuthenticated && (
              <Button 
                variant="primary"
      ...
      {/* Modais - renderizados apenas se o usuário estiver autenticado */}
      {isAuthenticated && renderModalNovoGerenciamento()}
      {isAuthenticated && renderModalNovoItem()}
      {isAuthenticated && renderModalNovaAcao()}
      {isAuthenticated && renderModalPermissoes()}
      {isAuthenticated && renderModalConfirmDelete()}
      {isAuthenticated && renderModalToggleChecklist()}
    </Container>
  );
};
